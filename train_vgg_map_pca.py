...
from network.vgg_pca_new import vgg16
...
def train_xcp_orig(batch, label, mask):
    model_xcp.train()
    model_xcp.is_training = True
    map_, y, proj, feat_mean = model_xcp(batch)
    ...
    loss.backward()
    optimizer_xcp.step()
    return[c_loss.item()], [p_loss.item()], real_pca_map, fake_pca_map, proj, feat_mean


def test_orig(batch, label, proj, feat_mean):
    model_xcp.eval()            # Change
    model_xcp.is_training = False
    x_, n = model_xcp(batch, proj, feat_mean)        # Change
...
while itr != opt.it_end+1:
    ...
    if itr % 10 == 0:
        ...
        for i in range(5):
            ...
            a, b = test_orig(batch, label, proj, feat_mean)
            test_results[0] += a
            test_results[1] += b
